...
function DashboardHeaderContent({ onSearch }: DashboardHeaderProps) {
  ...
  return (
    <div className="z-50">
      {/* Desktop Header */}
      <nav className="hidden md:flex items-center z-[9999] border-b border-cyan-200 h-16 ml-64 bg-gradient-to-r from-[#F5D5D0] via-white to-[#D3E8F0]">
        <div className="flex-1 flex items-center justify-between px-6">
          {/* Left Section - Search */}
          <div className="relative flex-1 max-w-lg" ref={dropdownRef}>
            {/* Search Input with Type Selector Inside */}
            <div className="relative flex items-center">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-cyan-400 z-10" />
              <Input
                type="text"
                value={searchQuery}
                ...
                className="pl-9 pr-28 h-9 bg-white border-cyan-200 focus-visible:ring-1 focus-visible:ring-primary text-sm"
              />
              {isFetching && (
              ...
              <div className="absolute right-1 top-1/2 -translate-y-1/2" ref={typeDropdownRef}>
                <button
                  ...
                  className="flex items-center gap-1 h-7 px-2 bg-white border border-cyan-200 rounded text-xs text-foreground/70 hover:bg-cyan-50 transition-colors"
                >
...
export default function DashboardHeader(props: DashboardHeaderProps) {
  return (
    <Suspense fallback={
      <nav className="hidden md:flex items-center fixed top-0 right-0 left-0 z-[9999] border-b border-cyan-200 h-16 ml-64 bg-gradient-to-r from-[#F5D5D0] via-white to-[#D3E8F0]">
        <div className="flex-1 flex items-center justify-between px-6">
