...
import { generateHealiumReportHTML } from "@/lib/services/report-generator"
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ case_id: string }> }) {
  try {
    ...
    const {data: dudd, error: erirfj} = await supabase
      .from("cases")
      .update({
        analyzed_by_ai: true,
        ai_analysis_status: "completed",
      ...
      console.log('erirf',erirfj)
      console.log('fhfh',dudd)
